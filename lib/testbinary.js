...
function testbinary(gyp, argv, callback) {
    ...
    try {
      ...
      console.log('Array.isArray(targets):', Array.isArray(targets));

      if (!(targets || Array.isArray(targets))) {
        console.log('JUST ONE TARGET!');
        targets = [binary_module];
        console.log('Trying to require binary 1:', req_path);
        req_path = binary_module;
        console.log('Trying to require binary 2:', req_path);
      } else {
        req_path = path.join(req_path, 'lib', platform, 'bindings');
      }

      console.log('Trying to require binary 3:', req_path);
